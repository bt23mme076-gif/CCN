  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl p-6 sm:p-8 max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-4 sm:mb-6 rounded-xl border border-blue-100 bg-blue-50/70 p-3 sm:p-4">
          <p className="text-xs sm:text-sm font-semibold text-blue-900 mb-3">
            Supported UPI apps
          </p>
          <div className="grid grid-cols-4 gap-2 text-[10px] sm:text-xs font-semibold text-slate-700">
            {[
              { label: 'UPI', bg: 'from-indigo-500 to-violet-500' },
              { label: 'GPay', bg: 'from-blue-500 to-cyan-500' },
              { label: 'PhonePe', bg: 'from-purple-600 to-fuchsia-500' },
              { label: 'Paytm', bg: 'from-slate-700 to-slate-900' },
            ].map((item) => (
              <div
                key={item.label}
                className="flex flex-col items-center gap-2 rounded-lg bg-white/90 border border-white shadow-sm px-2 py-2"
              >
                <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${item.bg} text-white flex items-center justify-center text-[11px] sm:text-xs font-black shadow-md`}>
