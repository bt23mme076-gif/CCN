...
function buildWhatsAppLink(name: string, mobile: string, planName: string, expiresAt: string) {
  ...
  const phone = mobile.replace(/\D/g, '').replace(/^0/, '91');
                ...
                {items.map((item) => {
                  ...
                  return (
                    <tr key={item.rechargeId}>
                      ...
                      <td>
                        <a
                          href={buildWhatsAppLink(item.customerName, item.customerMobile, item.planName, item.expiresAt)}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-xs font-bold text-white transition-all hover:scale-105"
